...
const nobot = "6285702255815" // Ganti No Bot Kalian
const namabot = "AzzBot MD" // Ganti Nama Bot Kalian
const nameown = "YahyaGanzz" // Ganti Nama Kalian
const numown = "081511480762" // Ganti Nomor Kalian
const nameowner = "YahyaGanzz" // Ganti Nama Kalian
const footer = "AzzBot MD 2022" // Ganti Nama Bot Kalian
const capt = "*By AzzBot MD*" // Ganti Nama Bot Kalian
const grup = "https://chat.whatsapp.com/J2HWEw6ni111WIpbOpNOmG" // Ganti Link Group Kalian
const nomorown = "https://wa.me/6281511480762" // Ganti Nomor Kalian
const note = "*_Bot Ini Masih Dalam Pengembangan Jika Ada Fitur Yang Eror Silakan Hubungi Owner!!!_*" // Kalau Mau Ganti Aja
const namagrup = "Group AzzBot MD" // Ubah Nama Group Mu

// Setting Donasi
const gopay = "-" // Ganti Nomor Gopay Mu
const pulsa = "081511480762" // Ganti No Pulsa Mu
const insta = "KAZUYAKUN" // Ganti Nama IG Mu
const github = "AzzBott679" // Ganti Nama Github Mu
const linkdonasi = "https://github.com/AzzBott679" // Ganti Link Qris Mu

// Setting SewaBot Ubah Harga Sesuai Harga Mu//
const qris = ""
const nomor = "wa.me/6281511480762"
...
let antitt = JSON.parse(fs.readFileSync('./database/antitt.json'));
...
module.exports = async(conn, msg, m, setting, store) => {
    ...
    const readmore = more.repeat(4001)
        ...
        const isAntiTt = isGroup ? antitt.includes(from) : false
		...
		const buttonsAntitt = [
			{ quickReplyButton: { displayText: `Antitt On`, id: `${prefix}Antitta enable` } },
			{ quickReplyButton: { displayText: `Antitt Off`, id: `${prefix}Antitta disable` } },
		]
		...
		const buttonsAntiyt = [
			{ quickReplyButton: { displayText: `Antiyt On`, id: `${prefix}antiyta enable` } },
			{ quickReplyButton: { displayText: `Antiyt Off`, id: `${prefix}antiyta disable` } },
		]
      ...
      conn.groupParticipantsUpdate(from, [number], "remove")
        ...
        //Anti Yt
            ...
            if (chats.match(`://youtube.com`)) {

                reply(`*[ YOUTUBE LINK DETECTOR ]*\n\nSepertinya kamu mengirimkan link yutube, maaf kamu akan di kick`)
...
if (chats.startsWith("@6287898635798")){
...
saya ${namabot}, bot ini adalah Multi-Device Whatsapp
...
🗓️Tanggal Server : ${moment.tz('Asia/Jakarta').format('DD/MM/YY')}
...
case prefix+'donasi':
  case prefix+'donate':
  var donasibut = [
			{ urlButton: { displayText: `Group AzzBot`, url : `${grup}` } },
...
_Mau Beli Sc No Enc? Ketik (.owner)_ `
...
*_Pembayaran Melalui Gopay&Pulsa_*
 _Pulsa:081511480762 +5k_
 _Gopay:081511480762_
...
case prefix+'infobot':
  case prefix+'inforobot':
    case prefix+'info':
      var caption = `*[ INFO AzzBot ]*

*Nama Bot :* AzzBot-Md
*Name Owner :* YahyaGanzz
...
break
  ...
  ├─ ❏ -
  ├─ ❏ INSTAGRAM
  └─ ❏ https://www.instagram.com/KAZUYAKUN
...
case prefix+'githubown':
 ...
 break
			...
			{ quickReplyButton: { displayText: `Ubah Ke Audio`, id: `${prefix}tiktokaudio ${q}` } },
				...
				reply(mess.wait)
				conn.sendMessage(from, { caption: `Succes Download Video Tiktok, Thanks For Using AzzBot-MD`, video: {url: data.nowm}, templateButtons: tidtod, footer: `${footer}`, mentions: [sender]} )
			}).catch(() => reply(mess.error.api))
			limitAdd(sender, limit)
			    break
...
case prefix+'bc': case prefix+'broadcast':
			    if (!isOwner) return reply(mess.OnlyOwner)
		            if (args.length < 2) return reply(`Masukkan isi pesannya`)
                            ...
                            for (let i of data) {
                               conn.sendMessage(i.id, { text: `*[ AzzBot BROADCAST ]*\n\n${q}` })
                               await sleep(1000)
                            }
                            break
...
case prefix+'linkig':
if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
if (args.length < 2) return reply(`Kirim perintah ${command} KAZUYAKUN`)
reply(`*[ INSTAGRAM ]*\n*_Link : https://instagram.com/${q}_*`)
limitAdd(sender, limit)
break
case prefix+'github':
if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
if (args.length < 2) return reply(`Kirim perintah ${command} AzzBott679`)
...
conn.sendMessage(from, { text: slotnya, buttons: but, footer: "© Slot By YahyaGanzz\n@AzzBot", templateButtons: but }, {quoted: msg})
gameAdd(sender, glimit)
        break
case prefix+'cekme':
  case prefix+'me':
...
case prefix+'antilink':
 ...
 conn.sendMessage(from, { text: teks, templateButtons: buttonsAntilink, footer: 'Anti Link Wa', mentions: [sender]} )  
 break
 case prefix+'antitt':
 var teks = `\n\n*Mode Antitt Silakan Pilih On/Off*\n\n`
 conn.sendMessage(from, { text: teks, templateButtons: buttonsAntitt, footer: 'Anti TikTok', mentions: [sender]} )  
 break
 case prefix+'antiyt':
 var teks = `\n\n*Mode Antiyt Silakan Pilih On/Off*\n\n`
 conn.sendMessage(from, { text: teks, templateButtons: buttonsAntiyt, footer: 'Anti Youtube', mentions: [sender]} )  
 break
...
case prefix+'antilinka':
                ...
                if (args[1].toLowerCase() === 'enable'){
					...
					reply('*Antitt Grup Aktif*')

                } else if (args[1].toLowerCase() === 'disable'){

                    let anu = antitt.indexOf(from)

                    antitt.splice(anu, 1)

                    fs.writeFileSync('./database/antitt.json', JSON.stringify(antitt))

                    reply('*Antitt Grup Nonaktif*')
